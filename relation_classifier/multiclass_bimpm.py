...
class BiMpm(Model):
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        for _class in range(len(self.class_weights)):
            classname = self.vocab.get_token_from_index(_class, namespace="labels")
            metrics.update({
                f"f1_{classname}": self.metrics[f"f1_rel{_class}"].get_metric(reset=reset)['f1'],
            })

        metrics["f1_macro"] = numpy.mean([metrics[key] for key in metrics.keys() if 'f1' in key])
        return metrics
