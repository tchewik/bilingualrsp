...
import numpy as np
import pandas as pd
from isanlp.annotation import Token, Span
...
class FeaturesProcessor:
    ...
    def __init__(self,
        ...
        if self._use_use:
            # pip install tensorflow tensorflow_text tensorflow-hub
    ...
    def tokenize(self, text, start):
        ...
        return get_tokens(start, len(text.strip()))
    ...
    def __call__(self, df_, annot_text, annot_tokens, annot_sentences, annot_lemma, annot_morph, annot_postag,
        ...
        df = df_.copy()
        # if self._whitespace_tokenization:
        #     df.snippet_x_tokens = df.snippet_x.map(lambda row: self.whitespace_tokenize(row.split()))
        #     df.snippet_y_tokens = df.snippet_y.map(lambda row: self.whitespace_tokenize(row.split()))
        # else:
    ...
    def _get_fpos_vectors(self, row):
        ...
        #return row.append()
        return pd.concat([row, pd.Series(list(result.values()), index=list(result.keys()))])
    ...
    def _linguistic_features(self, row):
        ...
        return pd.concat([row, pd.Series(list(tags.values()), index=list(tags.keys()))])
