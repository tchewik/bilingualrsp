...
class Predictor:
    def __init__(self, model_dir: str, cuda_device: int):

        self.model_path = os.path.join(model_dir, 'best_weights.pt')
        self.config_path = os.path.join(model_dir, 'config.json')
        self.config = json.load(open(self.config_path))

        self.data_manager = DataManager(corpus=self.config['data']['corpus']
                                        ).from_pickle(self.config['data']['data_manager_path'])
    ...
    def _load_model(self):
        ...
        model_config = {
            'relation_table': self.data_manager.relation_table,
            'classes_number': len(self.data_manager.relation_table),
