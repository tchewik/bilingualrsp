...
def bracketize_text(text, force_brackets):
    result = text.strip().replace('-RB-', ')').replace('-LB-', '(').replace('##### ', '\n')
    if force_brackets:
        # Somehow there is a collision in RST-DT
        while '  ' in result:
...
def datastructure2isanlpdu(tree, force_brackets=False):
    global id
    id += 1
    ...
    if tree.lnode:
        ...
        if tree.rnode.nucedu:
            ...
            edu_right = DiscourseUnit(
                ...
                relation='elementary',
                text=bracketize_text(tree.rnode.text, force_brackets)
            )
        ...
        new_unit = DiscourseUnit(
            ...
            relation='elementary',
            text=bracketize_text(tree.text, force_brackets)
        )
...
def convert2isanlp(rst, force_brackets):
    global id
    id = rst.tree.eduspan[-1]  # number of edus in tree
    return datastructure2isanlpdu(rst.tree, force_brackets)


def read_dis(filename, force_brackets=False):
    rst = RSTTree(filename)
    rst.build()
    return convert2isanlp(rst, force_brackets=force_brackets)
